...
namespace com.glups.Reward
{
    public class RewardModel
    {
        ...
        // display 
        internal virtual void trace(int date, string playerID, string strategy)
        {
            Debug.Log("\t" + date + ", " + playerID +"." + PlayerPrefs.GetString("Player") + "." + PlayerPrefs.GetString("name:" + PlayerPrefs.GetString("Player")) + ", " + strategy + ", " + "<level " + _currentLevel + "> " + "<Score " + _score + "> <rank " + _rank + "> " + "<iScore " + _iScore + ", " + string.Format("{0:F04}", _incentive) + ", " + _nextThreshold + "> <reward " + _reward + "> " + "<positives " + _positives + "> <negatives " + _negatives + ">");
        }
        ...
        public virtual void openLevel(int level)
        {
            ...
            _positives = 0;
            _negatives = 0;
            _score = PlayerPrefs.GetInt("Player" + PlayerPrefs.GetString("Player") + "scoreTotal");
            _iScore = PlayerPrefs.GetInt("Player" + PlayerPrefs.GetString("Player") + "RewardScore");
            _reward = PlayerPrefs.GetInt("Player" + PlayerPrefs.GetString("Player") + "RewardCounter");
            _incentive = 0.001f;
            _previousThreshold = _iScore;
            _nextThreshold = PlayerPrefs.GetInt("Player" + PlayerPrefs.GetString("Player") + "NextThreshold");
        }
        ...
        internal virtual int addPositive(int count, int points)
        {
            ...
            PlayerPrefs.SetInt("Player" + PlayerPrefs.GetString("Player") + "scoreTotal", _score);
            PlayerPrefs.SetInt("Player" + PlayerPrefs.GetString("Player") + "RewardScore", _iScore);
            return _iScore;
        }
        ...
        public virtual int updateRank(int[] steps)
        {
            ...
            // correct the _rank if it is higher then what it should be.
            while (_rank > 0 && _score < steps[_rank - 1])
            {
                _rank--;
                PlayerPrefs.SetInt("Player" + PlayerPrefs.GetString("Player") + "Rank", _rank);
            }
        ...
        internal virtual int updateReward(int gap)
        {
            ...
            if (_nextThreshold == _previousThreshold)
            {
                ...
                PlayerPrefs.SetInt("Player" + PlayerPrefs.GetString("Player") + "NextThreshold", _nextThreshold);
            }
            ...
            if (distance > 0)
            {
                ...
                _reward++;
                PlayerPrefs.SetInt("Player" + PlayerPrefs.GetString("Player") + "RewardCounter", _reward);
                PlayerPrefs.SetInt("Player" + PlayerPrefs.GetString("Player") + "NextThreshold", _nextThreshold);
                return _reward;
            }
            else
